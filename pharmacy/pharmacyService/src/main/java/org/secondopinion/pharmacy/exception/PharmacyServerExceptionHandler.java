...
public class PharmacyServerExceptionHandler {
	private static Logger logger = LoggerFactory.getLogger(PharmacyServerExceptionHandler.class);
	@Autowired
	...
	@ExceptionHandler(value = PharmacyServerException.class)
	public ResponseEntity<Response<String>> handleGenericException(PharmacyServerException dse, Exception ex) {
		Response<String> response = new Response<>();
		response.setMessage(dse.getMessage());
		response.setStatus(StatusEnum.FAILURE.name());
		// sending email to admin user
		loggingService.sendErrorMessageMail(mailProperties, "Exception in Pharmacy", appProperties.getSmtp().getErrorMessageEmail(), ex);
